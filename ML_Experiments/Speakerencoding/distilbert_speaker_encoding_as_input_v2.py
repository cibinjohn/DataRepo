# -*- coding: utf-8 -*-
"""distilbert speaker encoding as input v2.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1WqkVjhO3f6O33jArO7lWLn5kZfljItmx
"""
...
class InputExample:
    ...
    def __init__(self, guid: str, words: List[str], labels: List[str], speaker_ids: List[int] = None):
        self.guid = guid
        self.words = words
        self.labels = labels
...
class InputFeatures:
    ...
    def __init__(self, input_ids, attention_mask, label_ids, speaker_ids):
        self.input_ids = input_ids
        self.attention_mask = attention_mask
        self.label_ids = label_ids
        self.speaker_ids = speaker_ids

"""# Custom Model with Speaker Embeddings - NEW ARCHITECTURE"""
class DistilBertForTokenClassificationWithSpeaker(DistilBertForTokenClassification):
    """DistilBERT with speaker embeddings for token classification

    Architecture: Speaker embeddings are added to the input embeddings BEFORE
    passing through DistilBERT, similar to how positional embeddings work in transformers.
    """

    def __init__(self, config, num_speakers=3):
...
class DistilBertNERTraining:
    """
    Class for training DistilBERT-based NER models with speaker embeddings
    ...
    def __init__(
        ...
        # Special speaker ID for padding and special tokens (use 2 since speakers are 0 and 1)
        self.pad_speaker_id = 0
    ...
    def load_pretrained_model(self, num_speakers=3):
        """Load pretrained DistilBERT model and tokenizer with speaker embeddings

        Args:
            num_speakers: Number of speaker IDs (default 3: speaker 0, speaker 1, and padding/special token ID 2)
        """
        ...
        # Create custom model with speaker embeddings
        self.model = DistilBertForTokenClassificationWithSpeaker(
            config=model_config,
        ...
        logger.info(f"DistilBERT model with speaker embeddings loaded successfully (num_speakers={num_speakers})")

    def load_saved_model(self, model_dir: Optional[str] = None, num_speakers=3):
    ...
    def _read_examples_from_file(self, file_path: str, mode: str = "train") -> List[InputExample]:
        """
        Read examples from a CoNLL-format file with speaker IDs
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            for line_num, line in enumerate(f, 1):
                ...
                if line.startswith("-DOCSTART-") or line == "":
                    ...
                    splits = line.split()
                    if len(splits) >= 3:  # speaker_id, token, label
                        try:
                            speaker_ids.append(int(splits[0]))
                            words.append(splits[1])
                            labels.append(splits[2])
                        except ValueError as e:
                            logger.warning(f"Line {line_num}: Could not parse speaker_id '{splits[0]}' as integer. Line: {line}")
                    elif len(splits) == 2:
                        logger.warning(f"Line {line_num}: Only 2 columns found (missing speaker_id). Defaulting to speaker_id=0. Line: {line}")
        ...
        if len(examples) == 0:
            ...
            logger.error("Expected format: 'speaker_id token label' (3 columns separated by spaces)")
        return examples
    ...
    def _convert_examples_to_features(
        ...
        """
        Convert examples to features suitable for DistilBERT with speaker embeddings
        ...
        for ex_index, example in enumerate(tqdm(examples, desc="Converting examples")):
            ...
            # Tokenize each word and align labels and speaker IDs
            for word, label, speaker_id in zip(example.words, example.labels, example.speaker_ids):
            ...
            # Add special tokens - use pad_speaker_id (2) for special tokens
            ...
            # Pad to max length - use pad_speaker_id (2) for padding
    ...
    def _create_dataloader(
        ...
        dataset = TensorDataset(all_input_ids, all_attention_mask, all_label_ids, all_speaker_ids)
    ...
    def evaluate(self, eval_file: str, return_predictions: bool = True) -> Tuple[Dict[str, float], Optional[str]]:
        ...
        Returns:
            ...
            - predictions_text: String with "speaker_id token prediction" format (None if return_predictions=False)
        """
        if self.model is None or self.tokenizer is None:
        ...
        if return_predictions:
            # Read original tokens and speaker IDs from file
            original_data = []
            with open(eval_file, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    if line.startswith("-DOCSTART-") or line == "":
                        ...
                        splits = line.split()
                        if len(splits) >= 3:
                            original_data.append((splits[0], splits[1], splits[2]))
                        elif len(splits) == 2:
                            original_data.append(("0", splits[0], splits[1]))
            ...
            # Create predictions text with speaker_id token prediction format
            predictions_lines = []
            pred_idx = 0

            for speaker_id, token, label in original_data:
                if token == "":
                    predictions_lines.append("")
                else:
                    if pred_idx < len(flat_predictions) and flat_predictions[pred_idx] != "":
                        predictions_lines.append(f"{speaker_id} {token} {flat_predictions[pred_idx]}")
                        pred_idx += 1
                    else:
                        predictions_lines.append(f"{speaker_id} {token} O")
...
trainer = DistilBertNERTraining(
    labels=labels,
    model_save_dir="drive/MyDrive/exl/model/SE2",
    config=config
)
...
# Train
trainer.train(
    train_file="drive/MyDrive/exl/data/speaker_encoding_v3/train.txt"
    # dev_file="./data/dev.txt"  # Optional
)

results, predictions = trainer.evaluate("drive/MyDrive/exl/data/speaker_encoding/test.txt", return_predictions=True)
